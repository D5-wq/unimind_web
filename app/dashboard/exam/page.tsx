      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          messages: [
            {
              role: "user",
              content: `다음 시험 문제에 대한 학생의 답변을 평가해줘.
...
짧게 (3-4줄) 피드백을 주고, 마지막 줄에 반드시 "평가: 정확 / 부분적 / 틀림" 중 하나만 써줘.`,
      ...
      const score: "correct" | "partial" | "wrong" =
        fb.includes("정확") ? "correct" :
        fb.includes("부분적") ? "partial" : "wrong"
      setScores(prev => {
        const next = [...prev]
        next[index] = score
        return next
      ...
      <Card className="w-full max-w-2xl rounded-2xl shadow-2xl">
        <CardContent className="p-6">
            ...
            {scores.map((s, i) => (
              <div
                ...
                className={cn(
                  ...
                  i === index ? "bg-primary" :
                  s === "correct" ? "bg-green-500" :
                  s === "partial" ? "bg-yellow-500" :
                  s === "wrong" ? "bg-destructive" :
                  "bg-secondary"
                )}
              />
            ))}
          ...
          <textarea
            ...
            className={cn(
              "mb-3 w-full resize-none rounded-xl border border-border bg-card p-4 text-sm leading-relaxed outline-none transition-colors focus:border-primary/50",
              feedback ? "opacity-60" : ""
            )}
            rows={5}
          />
          ...
          <div className="flex items-center justify-between">
            ...
            {!feedback ? (
              <Button className="rounded-xl px-6" onClick={submit} disabled={!answer.trim() || loading}>
                {loading
                  ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />채점 중...</>
                  : <><Send className="mr-2 h-4 w-4" />채점하기</>}
              </Button>
            ) : index < total - 1 ? (
              <Button className="rounded-xl px-6" onClick={next}>
                다음 문제<ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            ) : (
              <Button className="rounded-xl px-6 bg-green-600 hover:bg-green-700" onClick={onClose}>
                퀴즈 완료! 🎉
              </Button>
            )}
          </div>
